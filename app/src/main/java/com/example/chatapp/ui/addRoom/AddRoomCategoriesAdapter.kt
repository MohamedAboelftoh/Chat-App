...
class AddRoomCategoriesAdapter(val items :List<Category>):BaseAdapter() {
    ...
    override fun getView(position: Int, view: View?, parent: ViewGroup?): View {
        ...
        if(view==null){
            //create viewHolder
            val itemBinding = ItemRoomCategoryBinding.inflate(LayoutInflater.from(parent?.context),parent,false)
    ...
    class ViewHolder(val itemBinding:ItemRoomCategoryBinding){
        fun bind(item: Category){
            itemBinding.image.setImageResource(item.img)
